...
{
    public partial class FailedChecklists : System.Web.UI.Page
    {
        ...
        public void DisplayRecord()
        {
            ...
            string SQL = "SELECT * FROM[CASE_INFO] INNER JOIN[PROPERTY_INFO] ON[CASE_INFO].Property_ID = [PROPERTY_INFO].Property_ID Where[CASE_INFO].Inspection_Status = 'Failed'";
...

            {
                string SQL1 = "SELECT CASE_INFO.Case_Num,CASE_INFO.Inspection_Date, CASE_INFO.Inspection_Status, INSPECTOR_INFO.Inspector_FName+' '+INSPECTOR_INFO.Inspector_LName as Inspector, PROPERTY_INFO.Property_StreetNumber+' '+PROPERTY_INFO.Property_StreetName+' '+PROPERTY_INFO.Property_Zip as Address" +
                        " FROM CASE_INFO left join INSPECTOR_INFO ON CASE_INFO.Inspector_ID=INSPECTOR_INFO.Inspector_ID left join PROPERTY_INFO ON CASE_INFO.Property_ID = PROPERTY_INFO.Property_ID Where[CASE_INFO].Inspection_Status = 'Failed'";
